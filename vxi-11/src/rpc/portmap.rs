...
pub(crate) mod xdr {
    ...
    impl RpcProto {
        pub(crate) fn prot(&self) -> u32 {
            match self {
                RpcProto::Tcp => xdr::IPPROTO_TCP as u32,
                RpcProto::Udp => xdr::IPPROTO_UDP as u32,
            }
        }
    }
...
pub(crate) struct PortmapClient<CLIENT> {
    client: CLIENT,
}

impl<CLIENT: RpcClient> PortmapClient<CLIENT> {
    async fn set(
        &mut self,
        prog: u32,
        vers: u32,
...
impl PortmapClient<RpcTcpClient> {
    fn new() -> Self {
        PortmapClient {
...
#[cfg(test)]
mod tests {
    use crate::rpc::portmap::PortmapClient;
    ...
    async fn test_portmap_tcp() {
        let mut client = PortmapClient::new();
